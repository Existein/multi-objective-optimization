...
# Lambda Function URL
LAMBDA_FUNCTION_URL = "https://cymhqhrghtfydfwfbwkgdminmu0mlncj.lambda-url.us-east-1.on.aws/"
...
def run_benchmark():
    log_group = "/aws/lambda/cpu_intensive_benchamark"  # 로그 그룹 설정
    
    # Lambda 함수 실행만 진행
    for memory in config_list:
        update_lambda_memory("cpu_intensive_benchamark", memory)  # 메모리 크기 변경 
    ...
    # Lambda 함수 호출이 모두 완료된 후, 일정 시간 대기 후 로그에서 실행 시간 추출
    print("Waiting for all logs to be available...")
    time.sleep(100)  # 로그가 모두 생성될 시간을 충분히 기다림

    # 모든 실행 시간 추출
    all_execution_times = get_all_execution_times(log_group)

    # 결과를 저장
    results = []
    for memory, exec_times in zip(config_list, [all_execution_times[i:i+10] for i in range(0, len(all_execution_times), 10)]):
        average_execution_time = sum(exec_times) / len(exec_times)
        results.append({
            'MemorySize': memory,
            'AverageExecutionTime': average_execution_time,
            'ExecutionTimes': exec_times
        })
        print(f"Memory {memory}MB, Average Execution Time: {average_execution_time} ms")
